...
internal class ResultBusinessHandler
{
    public static Result GetResultFromCards(IEnumerable<Card> cards)
    {
        var result = StraightFlush(cards);
        if (result.Rating > 0) return result;
    ...
    private static Result HighCard(IEnumerable<Card> cards)
    {
    ...
    private static Result StraightFlush(IEnumerable<Card> cards)
    {
        var straightResult = Straight(cards);
        if (Flush(cards).Rating > 0 && straightResult.Rating > 0)
            return straightResult.Rating == 0.6m
                ? new Result(9, ResultKind.RoyalFlush, "royal flush")
                : new Result(9, ResultKind.StraightFlush, straightResult.Message.Replace("straight", "straight flush"));
        return new Result(0, ResultKind.HighCard, "");
    }

    private static Result Flush(IEnumerable<Card> cards)
    {
        ...
        var rating = 5m;
        foreach (var card in flushGroup) rating += (decimal)card.Value / 1000;
    ...
    private static Result Straight(IEnumerable<Card> cards)
    {
        ...
        {
            if (card == highest - straightCount ||
                highest == CardValue.Five && card == CardValue.Ace && straightCount == 4)
            {
                straightCount++;
            }
            else
    ...
    private static Result FullHouse(IEnumerable<Card> cards)
    {
        var oneList = cards.GroupBy(x => x.Value).Where(x => x.Count() == 3).OrderByDescending(x => x.Key)
            .FirstOrDefault();
        if (oneList == null) return new Result(0, ResultKind.HighCard, "");
        var second = cards.GroupBy(x => x.Value).Where(x => x.Key != oneList.Key).Where(x => x.Count() == 2)
    ...
    private static Result FourOfAKind(IEnumerable<Card> cards)
    {
        var oneList = cards.GroupBy(x => x.Value).Where(x => x.Count() == 4).OrderByDescending(x => x.Key)
            .FirstOrDefault();
        if (oneList == null) return new Result(0, ResultKind.HighCard, "");
        var rating = 7 + (decimal)oneList.Key / 100;
        cards.Where(x => x.Value != oneList.Key).OrderByDescending(x => x.Value).Take(1)
    ...
    private static Result ThreeOfKind(IEnumerable<Card> cards)
    {
        var oneList = cards.GroupBy(x => x.Value).Where(x => x.Count() == 3).OrderByDescending(x => x.Key)
            .FirstOrDefault();
        if (oneList == null) return new Result(0, ResultKind.HighCard, "");
    ...
    private static Result TwoPair(IEnumerable<Card> cards)
    {
        ...
        var rating = 2 + (decimal)twoLists.First().ToList().First().Value / 100 +
                     (decimal)twoLists.Last().ToList().First().Value / 10000 + (decimal)cards
                         .OrderByDescending(x => x.Value).First(x => !twoLists.Any(i => i.Key == x.Value)).Value /
                     1000000;
        twoLists.First().Key.ToString();
        return new Result(rating, ResultKind.TwoPair,
            $"two pair, {twoLists.First().Key.ToString()}s and {twoLists.Last().Key.ToString()}s");
    }

    private static Result OnePair(IEnumerable<Card> cards)
    {
        var oneList = cards.GroupBy(x => x.Value).Where(x => x.Count() == 2).OrderByDescending(x => x.Key)
            .FirstOrDefault();
        if (oneList == null) return new Result(0, ResultKind.HighCard, "");
        var rating = 1 + (decimal)oneList.Key / 100;
        var highCards = cards.Where(x => x.Value != oneList.Key).OrderByDescending(x => x.Value).Take(3);
        rating += highCards.Sum(c => (decimal)c.Value / 100000);
